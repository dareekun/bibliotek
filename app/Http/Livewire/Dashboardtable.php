...
class Dashboardtable extends Component
{
    ...
    public function render()
    {
        if (Auth::user()->role == 'developer') {
            ...
            $this->valid    = DB::table('document')->where('pic', Auth::user()->email)->where('statusdoc', 1)->count();
            $this->pending  = DB::table('document')->where('pic', Auth::user()->email)->where('statusdoc', 2)->count();
            $this->ongoing  = DB::table('document')->where('pic', Auth::user()->email)->where('statusdoc', 3)->count();
            $this->wating   = DB::table('document')->where('pic', Auth::user()->email)->where('statusdoc', 4)->count();
            $this->deactive = DB::table('document')->where('pic', Auth::user()->email)->where('statusdoc', 0)->count();
        } elseif (Auth::user()->role == 'user'){
            $this->valid    = DB::table('document')->where('creator', Auth::user()->id)->orWhere('pic', Auth::user()->email)->where('statusdoc', 1)->count();
            $this->pending  = DB::table('document')->where('creator', Auth::user()->id)->orWhere('pic', Auth::user()->email)->where('statusdoc', 2)->count();
            $this->ongoing  = DB::table('document')->where('creator', Auth::user()->id)->orWhere('pic', Auth::user()->email)->where('statusdoc', 3)->count();
            $this->wating   = DB::table('document')->where('creator', Auth::user()->id)->orWhere('pic', Auth::user()->email)->where('statusdoc', 4)->count();
            $this->deactive = DB::table('document')->where('creator', Auth::user()->id)->orWhere('pic', Auth::user()->email)->where('statusdoc', 0)->count();
        }
